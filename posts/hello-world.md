...
封面不是圖片檔，而是在瀏覽器裡即時畫出來的單色圖：

1. 用光線步進（ray marching）算出立體形狀每一點的明暗；地形則用值噪聲產生高度，再畫上等高線。
2. 用 Bayer 8×8 門檻矩陣做有序抖動，把灰階轉成只有一種顏色的點。

形狀由 `posts.json` 的 `cover` 欄位指定，沒有指定時依文章 `id` 自動挑選。同一篇文章的封面永遠一樣，也不需要另外準備圖片。
